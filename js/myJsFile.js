        ...
        success: function(response) {
          ...
          if (Array.isArray(response) && response.length > 0) {
            ...
            response.forEach(item => {
              ...
              dataSet.push([
                `${serialNumber++}. <input type="checkbox" class="rowCheckbox" value="${item[0]}">` // Checkbox
                ,
                ...
                `<a target=\"_blank\" href=\"entryform.php?id="${item[0]}" class=\"btn btn-primary\"> <i class=\"fa fa-pencil-square-o\" aria-hidden=\"true\"></i> </a> <a type=\"button\" class='btn btn-danger' onclick='deleteData("${item[0]}")'> <i class=\"fa fa-remove\" aria-hidden=\"true\"></i> </a>`
              ]);
            });
          } else {
            alert("No data found!");
  ...
  $("#bulkDeleteButton").click(function() {
    ...
    if (confirm("Are you sure you want to delete the selected rows?")) {
      ...
      $.ajax({
        ...
        success: function(response) {
          $("#responseMessage").html('<p class="alert alert-success">Selected rows deleted successfully!</p>');
        ...
        error: function(xhr, status, error) {
          ...
          $("#responseMessage").html("<span style='color:red;'>Failed to delete selected rows. Please try again later.</span>");
        },
      });
    }
  });
